...
<template>
    <div class="flex items-center justify-center">
        <div class="relative group leading-7">
            <button @click="isOpen = !isOpen" ref="componentRef" id="dropdown-button" class="text-gray-700 tracking-wider max-md:focus:text-[red] md:text-gray-600 md:hover:text-gray-800 md:hover:bg-zinc-300/60 py-2 px-2 md:px-4 rounded-md inline-flex items-center">
                ...
                <span class="mr-1 max-md:hidden">{{ getConfigConst(`localesMetadata.${locale}.label`) }}</span>
            ...
            <div v-if="isOpen" ref="excludeRef" id="dropdown-menu" class="absolute right-0 md:right-1/2 md:translate-x-1/2 mt-2 tracking-wide w-max rounded-md shadow-md bg-white ring-1 ring-black ring-opacity-5 p-2 space-y-1 font-jost text-zinc-600">
                <a @click="onClickChangeLanguage(loc)" v-for="loc in Translations.supportedLocales" :aria-checked="locale == loc" class="flex items-center px-4 py-2 align-baseline max-md:focus:text-[red] md:hover:bg-zinc-300/60 md:hover:text-zinc-800 aria-checked:bg-zinc-300/60 aria-checked:text-zinc-800 cursor-pointer rounded-md">
