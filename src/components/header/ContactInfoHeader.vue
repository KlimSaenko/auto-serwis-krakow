<script setup lang="ts">
  ...
  const appointmentFormOpened = ref(false);
</script>

<template>
  <header class='border-box z-50 align-middle min-h-[70px] sticky top-0 shadow-md font-jost tracking-[0.06em] leading-5 bg-white'>
    <div class="max-md:hidden bg-zinc-800 text-gray-100 font-jost-thin font-extrabold text-[15px] tracking-widest">
      <div class="flex items-center relative py-2 mx-auto px-8 2xl:px-12 max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg 2xl:max-w-screen-xl">
        ...
        <ul class='flex items-center mx-1 space-x-6 leading-6 ml-auto'>
          ...
          <li class='flex items-center'>
            <RouterLink to="#map" v-scroll-to="'map'" class='flex items-center hover:text-[red] cursor-pointer'>
              <svg class="mr-3" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20" viewBox="0 0 256 256" xml:space="preserve">
              ...
              <span>
                {{ getConfigConst("corporateInfo.addressShort") }}
              </span>
            </RouterLink>
          </li>
    ...
    <div class='flex items-center left-0 right-0 py-3 2xl:py-4 justify-between md:gap-x-2 lg:gap-x-4 mx-auto px-5 md:px-8 2xl:px-12 max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg 2xl:max-w-screen-xl text-gray-500'>
      ...
      <nav :aria-expanded="menuExpanded" class='h-12 items-stretch md:!flex md:flex-auto bg-white max-md:fixed right-0 left-0 top-16 bottom-full aria-expanded:bottom-0 duration-500 md:ml-5 md:mr-2 font-bold text-[17px] max-md:text-xl max-md:overflow-hidden'>
        ...
        <ul class='max-md:py-4 max-md:px-3 md:flex md:items-center ml-auto max-md:block md:space-x-8 max-md:ml-7'>
          <li class='max-md:py-2 max-md:mt-2 inline-flex h-full'>
            <button @click="appointmentFormOpened = true" class="my-[2px] tracking-[0.06em] text-gray-100 hover:text-[red] bg-[red] border border-[red] hover:bg-white py-2 px-3 md:px-4 rounded-md inline-flex items-center">
            ...
            <!-- <RouterLink to="#map" v-scroll-to="'map'" class='flex items-center nav-button fill-gray-500 md:hover:text-gray-800 md:hover:fill-gray-800 leading-6 md:leading-8'>
              ...
              <p class="hidden max-md:block lg:block ml-3">{{ getConfigConst("corporateInfo.addressShort") }}</p>
            </RouterLink> -->
          </li>
        </ul>
      </nav>
  ...
  <AppointmentModal v-bind="{ appointmentFormOpened: appointmentFormOpened }" @close="appointmentFormOpened = false" />
</template>
