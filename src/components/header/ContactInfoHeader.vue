<script setup lang="ts">
    ...
    const menuButtonCloseElement = ref<SVGAnimationElement>();
    const menuButtonOpenElement = ref<SVGAnimationElement>();
    const openAppointmentModal = inject<(description?: string) => void>('openAppointmentModal');

    watch(menuExpanded, expanded => {
        if (expanded){
            menuButtonOpenElement.value?.beginElement();
        } else {
            menuButtonCloseElement.value?.beginElement();
        }
    });
            ...
            <button @click="menuExpanded = !menuExpanded" class="relative inline-flex items-center w-10 h-10 justify-center text-sm text-[red] rounded-lg md:hidden">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10" stroke="#eee" stroke-width=".7" fill="rgba(0,0,0,0)" stroke-linecap="round">
                    ...
                    <rect width="10" height="10" stroke="none">
                        <animate ref="menuButtonCloseElement" dur="2s" id="reverse" attributeName="width" begin="indefinite" />
                    </rect>
                    <rect width="10" height="10" stroke="none">
                        <animate ref="menuButtonOpenElement" dur="0.001s" id="start" attributeName="width" values="10;0" fill="freeze" begin="indefinite" />
            ...
            <nav :aria-expanded="menuExpanded" class='md:h-12 items-stretch md:!flex md:flex-auto bg-white max-md:fixed right-0 left-0 top-16 bottom-full aria-expanded:bottom-0 duration-500 md:ml-5 md:mr-2 text-[17px] max-md:text-xl max-md:overflow-hidden'>
                ...
                <ul class='max-md:py-4 max-md:px-3 md:flex mb-1 max-md:space-y-2 space-x-7 lg:space-x-9'>
                    <li class='max-md:py-2 ml-7 content-center'>
                        <router-link :to="{ name: 'home' }" @click.native="menuExpanded = false" active-class="active text-gray-800" class='nav-button relative block md:hover:text-gray-800 leading-6 md:leading-8'>
                            {{ $t("header.home") }}
                        </router-link>
                    </li>

                    <li class='max-md:py-2 content-center'>
                        <router-link :to="{ name: 'services' }" @click.native="menuExpanded = false" active-class="active text-gray-800" class='nav-button relative block md:hover:text-gray-800 leading-6 md:leading-8'>
                            {{ $t("header.services") }}
                        </router-link>
                    </li>

                    <li class='max-md:py-2 content-center'>
                        <router-link :to="{ name: 'media' }" @click.native="menuExpanded = false" active-class="active text-gray-800" class='nav-button relative block md:hover:text-gray-800 leading-6 md:leading-8'>
                            {{ $t("header.media") }}
                        </router-link>
                    </li>
                </ul>
