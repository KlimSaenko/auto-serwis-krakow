<script setup lang="ts">
    ...
    watch(form.phone, phone => form.phone.value = phone.replace(/\D/g, ''));
...
<template>
	<Transition name="modal-fade" appear>
		<div v-if="isModalOpened" class="fixed inset-0 w-full h-full p-0 m-0 bg-zinc-900/60 z-[999] flex">
				...
				<form @submit.prevent="validateForm">
					...
					<div class="my-6">
						...
						<div class="relative">
							...
							<input id="customerName" type="text" v-model="form.customerName.value" class="bg-gray-50 border tracking-wide border-gray-300 text-black rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2 pr-3" placeholder="Your Name">
						</div>
					</div>
					...
					<div class="my-6">
						...
						<div class="flex relative">
							<div ref="componentRef" @focusin="isLocationSelectorOpen = true" class="relative flex font-jost items-center cursor-pointer pl-10 pr-6 font-medium text-center bg-gray-100 text-gray-700 border border-gray-300 rounded-s-lg hover:bg-gray-200">
								...
								<span>+</span>
								<input id="phoneCode" type="tel" :size="form.phoneCode.value.length" v-model="form.phoneCode.value" autocomplete="tel-country-code tel-area-code" maxlength="4" class="tracking-wide cursor-text shadow-none ring-0 border-0 outline-none bg-transparent" />
							...
							<div class="relative w-full">
								<input id="phone" type="tel" v-model="form.phone.value" maxlength="12" autocomplete="tel tel-national" class="block tracking-wide p-2 pr-3 w-full text-black bg-gray-50 rounded-e-lg border-s-0 border border-gray-300" placeholder="123-456-7890" />
							</div>
						</div>
					</div>
					...
					<div class="my-6">
						...
						<div class="flex relative">
							...
							<div class="relative w-full">
								<input id="telegram" type="text" v-model="form.telegram.value" class="block tracking-wide p-2 pr-3 w-full text-black bg-gray-50 rounded-e-lg border-s-0 border border-gray-300" placeholder="YourTelegram" />
							</div>
						</div>
					</div>
					...
					<div class="my-6">
						...
						<div class="relative">
							...
							<input id="email" type="email" v-model="form.email.value" autocomplete="email" class="bg-gray-50 tracking-wide border border-gray-300 text-black rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2 pr-3" placeholder="your@email.com">
						</div>
					</div>

					<div class="my-6">
						<label for="problemDescription" class="block mb-2">{{ $t("modal.customerMessageLabel") }}</label>
						<textarea id="problemDescription" rows="4" v-model="preDescription" class="block tracking-wide px-3 py-2 w-full text-black bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" :placeholder="$t('modal.customerMessagePlaceholder')">oke</textarea>
					</div>
