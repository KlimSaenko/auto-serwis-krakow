    ...
    <section>
            ...
            <div class="flex w-full relative">
                ...
                <div class="max-sm:px-8 sm:px-2 w-full md:px-4 2xl:px-16 max-w-lg sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg 2xl:max-w-screen-xl max-md:rounded-2xl">
                    <div v-for="(images, rowIndex) in paredImages" :aria-colspan="imagesIndex"
                        ...
                        "
                        class="group/aria w-full font-jost grid grid-cols-2 md:grid-rows-[auto,auto] md:grid-cols-4 md:aria-[colspan='1']:grid-cols-[20fr,30fr,25fr,25fr] md:aria-[colspan='2']:grid-cols-[25fr,25fr,30fr,20fr] md:aria-[colspan='3']:grid-cols-[30fr,25fr,20fr,25fr] duration-[400ms] md:gap-x-3 xl:gap-x-4 overflow-hidden">
        ...
        <transition name="fade-image" mode="out-in">
			<div v-if="imagePreview" class="fixed inset-0 bg-zinc-950/65 z-[300] flex items-center justify-center">
                ...
                <button @click="imagePreview = undefined" class="absolute right-6 top-24 md:top-[8.5rem] w-16 h-16 text-gray-200 md:hover:text-white rounded-full p-2.5 hover:bg-white/20 active:bg-white/20 duration-150">
